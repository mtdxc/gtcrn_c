...
int main(int argc, char* argv[]) {
    ...
    float output_data[BLOCK_SHIFT]; // Buffer for output data
    for (size_t i = 0; i < samples; i+= BLOCK_SHIFT)
    {
        if (samples - i < BLOCK_SHIFT) {
            std::cerr << "Warning: Not enough samples left for a full block." << std::endl;
            break; // Not enough samples for a full block
        }
        int processed_length = gtcrn.Process(pcm+i, output_data, BLOCK_SHIFT);
